...
class TBIDILSTM_legacy(nn.Module):
    """
    A torch module implementing a bidirectional LSTM with a linear layer as decoder.
    Very much the same as kraken clstm/separate-derivs BIDILSTM (hopefully). 
    """
    def __init__(self, ninp, nhid, nop):
        super(TBIDILSTM_legacy, self).__init__()
        
        self.rnn = nn.LSTM(ninp+1, nhid, 1, bias=False, bidirectional=True)
        self.decoder = nn.Linear(2*nhid, nop, bias=False)
    ...
    def forward(self, inp, hidden):
        ...
        decoded = self.decoder(lstm_out.view(lstm_out.size(0)*lstm_out.size(1), lstm_out.size(2)) )
...
class TlstmSeqRecognizer_legacy(kraken.lib.lstm.SeqRecognizer):
    """
    Something like ClstmSeqRecognizer, using pytorch instead of clstm.
    It should behave like the kraken code built on clstm/separate-derivs.
    Training is not supported. Needs testing!
    """
    def __init__(self, fname='', normalize=kraken.lib.lstm.normalize_nfkc, cuda=torch.cuda.is_available()):
        self.fname = fname
        self.rnn = None
    ...
    def init_model(cls, ninput, nhidden, noutput, codec, normalize=kraken.lib.lstm.normalize_nfkc, cuda=torch.cuda.is_available()):
        ...
        self.codec = codec
        self.normalize = normalize
        self.rnn = TBIDILSTM_legacy(ninput, nhidden, noutput)
        self.cuda_available = cuda
        if self.cuda_available:
            self.cuda()
        return self
    ...
    def _load_model(self):
        ...
        # softmax
        weight_softm = torch.from_numpy(weights['softm']['.W'].astype('float32'))
