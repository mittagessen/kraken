...
The method implemented in kraken is based on the Order Relation Operator method
described in this `paper`_. This algorithm has minimal training data requirements and is
extremely fast but operates solely on geometric information and object classes.
Any reading order that can not be derived from relative positions between lines
and regions and their classes, in particular those requiring visual information
from the source image, will not be learnt well by this system. A newer method
called `Orli`_ jointly learns baseline detection and reading order and can be
used for layout analysis of more complex material.

.. _`paper`: https://ieeexplore.ieee.org/document/9413256
.. _`Orli`: https://github.com/mittagessen/orli
...
- Manifest options were renamed to ``--training-data`` and
  ``--evaluation-data``.
- ``--resume`` restores full trainer state; ``--load`` starts a fresh run from
  existing weights.
- ``ketos roadd`` is deprecated in favor of ``ketos convert``.
- ``--class-mapping-from-ckpt`` can import mappings directly from segmentation
  checkpoints.
...
each page. The annotations may be implicit, i.e., just the order of the
elements in the XML, or be present in the XML reading order elements
(``<ReadingOrder>``).
...
.. code-block:: console
    ...
    # deprecated
    $ ketos roadd -o seg_with_line_ro.mlmodel line_ro.mlmodel -i seg.mlmodel
    $ ketos roadd -o seg_with_line_and_region_ro.mlmodel region_ro.mlmodel \
...
There is no way to adjust the class taxonomy of an existing reading order model
to fit another dataset with different classes. On the other hand RO models
learn well from very small datasets so training from scratch is usually easily
possible.
