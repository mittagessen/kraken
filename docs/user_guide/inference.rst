...
``-f, --format-type <type>``
    ...
    * ``xml``, ``alto``, ``page``: Parses existing segmentation from XML files. Useful for "resume" workflows (e.g., running recognition on already segmented ALTO files).
...
* ``-a``: **ALTO XML** (Standard for archival).
* ``-x``: **PageXML**.
* ``-h``: **hOCR** (HTML).
...
* ``--mask <image>``: A binary image indicating areas to ignore during segmentation.
...
* ``-m, --model <file>``: Path to the recognition model. Supports both pickled (``.mlmodel``) and Safetensors (``.safetensors``) formats.
...
* ``--base-dir <dir>``: Forces a specific initial text direction when running the BiDi algorithm (e.g., ``L``, ``R``). This overrides direction hints from the segmentation step.
* ``--temperature <float>``: Adjusts the softmax temperature during decoding.

    * *Values < 1.0*: Sharpen the probability distribution (more confident, less diverse).
    * *Values > 1.0*: Smooth the distribution (useful for calibration or sampling).
...
``--precision <str>``
    ...
    * ``32``: Standard FP32.
    * ``16``, ``16-mixed``: FP16 (Standard half-precision).
...
``--num-line-workers <int>``
    Number of CPU processes used for pre-processing image batches before they are sent to the GPU.

Examples
--------

**Standard Inference (GPU, BFloat16)**
    Segment and recognize a folder of images, outputting ALTO XML.

    .. code-block:: bash

       kraken -I "scans/*.tif" -o .xml --device cuda:0 --precision bf16-mixed -a segment -bl ocr -m extensive_model.safetensors

**Recognition on Existing ALTO Files**
    Run a specific model on pre-segmented ALTO files, creating new files with the suffix of ``_ocr.xml``.

    .. code-block:: bash

       kraken -f xml -I "data/*.xml" -o _ocr.xml ocr -m specialized.mlmodel

**PDF Extraction and Processing**
    Process a PDF, extracting images on the fly and outputting text files.

    .. code-block:: bash

       kraken -i source.pdf output.txt -f pdf segment -bl ocr -m model.safetensors
