...
kraken is an automatic text recognition (ATR) package that can be trained
...
To train models for all of the above we require training data, i.e. examples of
...
The easiest way to install and use kraken is through `pip` in a virtual
environment. Alternatively, if you do not have a setup or do not wish to handle
virtual environments yourself you can use `pipx` (on Ubuntu/Debian):
...
The split between training and validation set is random, so it will change
between invocations of the `train` command. In most scenarios it is therefore
...
manifest files `train.lst` and `val.lst` containing 90% and 10% random pages of
the overall data:

.. code-block:: console
  
  $ ls -1 *.xml | sort -R > all.lst
  $ let val_size=$(($(wc -l all.lst| awk '{print $1}')/10))
  $ head -n $val_size all.lst > val.lst
  $ tail -n+$val_size all.lst > train.lst

The manifest files can be used in `ketos train` with the `-t` and `-e` options:
...
that can be adjusted, the most important ones being:

--output
        Sets the prefix for models generated during training. They will be
        saved as ``prefix_N.mlmodel``. After training is finished the best
        model, i.e. the model with the lowest error rate on the validation set,
        will be saved as ``prefix_best.mlmodel``.
--freq
        How often evaluation passes are run on the validation set. It is a
        ...
        frequency with which intermediate models will be saved to disk.
--load
...
making training runs of under an hour possible.
...
prevent `overfitting <https://en.wikipedia.org/wiki/Overfitting>` which causes
the model to use random noise in the training data for classification instead
of the general patterns of the material, reducing its real world accuracy.
...
.. code-block:: console
        ...
        │ 2  │ net       │ MultiParamSequential     │  4.1 M │ train │       [[1, 1, 120, 400], '?'] │   [[1, 169, 1, 50], '?'] │
        ...
        │ 21 │ net.O_18  │ LinSoftmax               │ 67.8 K │ train │   [[1, 400, 1, 50], '?', '?'] │   [[1, 169, 1, 50], '?'] │
        └────┴───────────┴──────────────────────────┴────────┴───────┴───────────────────────────────┴──────────────────────────┘
        ...
        stage 0/∞ ╸━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 7/405  0:00:12 • 0:08:20 0.79it/s train_loss_step: 289.742 early_stopping: 0/10 -inf
        ...

By now there should be a couple of models model_1.mlmodel,
model_2.mlmodel, ... in the directory the script was executed in. Lets
take a look at each part of the output. First, `ketos` prints the detected
devices and if they are used. Our system does not contain a GPU (or other
accelerators) so it will indicate `False` for all of them:
...
Next, a summary of the model that is going to be trained is printed. The
information here is mostly of interest to people adjusting the model
architecture through the built-in `VGSL <vgsl>`_ language and can be ignored in
...
Afterwards the actual training begins. As mentioned above training is divided
into epochs, the number of training iterations after which the model has been
shown each line in the training dataset once. After each epoch 
